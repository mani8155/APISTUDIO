...
# only use apistudio and meritplus this function
@app.post('/auth/token', response_model=schemas.Token)
async def login_for_access_token(form_data: Annotated[schemas.GenerateToken, Depends()], db: Session = Depends(get_db)):
    auth_data = db.query(AuthTokenGenerator).filter(AuthTokenGenerator.secret_key == form_data.secret_key).first()
    # print(auth_data)
    ...
    encodes = {"uid": auth_data.uid, "api_source": auth_data.api_source, "id": auth_data.id}
...
@app.post("/auth/api/filter_auth")
def filter_auth(request: schemas.FilterAuthSchema, db: Session = Depends(get_db)):
    print(schemas.FilterAuthSchema)
    fil_value = request.fil_val
    obj = db.query(AuthTokenGenerator).filter(AuthTokenGenerator.api_source == fil_value).all()
    return obj


@app.get("/auth/api/{id}")
def id_to_get_data(id: int, db: Session = Depends(get_db)):
    obj = db.query(AuthTokenGenerator).filter(AuthTokenGenerator.id == id).first()
    return obj
