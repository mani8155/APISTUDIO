...
@app.get('/sqlviews/api/v1/trace_data/{id}')
def trace_data(id: int, db: Session = Depends(get_db)):
    obj = db.query(models.SqlViewsTrace).filter(models.SqlViewsTrace.table_id == id).all()
    return obj
...
def update(id: int, request: schemas.UpdateConnectionDbSchema, db: Session = Depends(get_db)):
    ...
    obj.api_trace = request.api_trace
    ...
    if obj.api_header == None:
        ...
        if obj:
            ...
            if rows:
                ...
                # trace_tbl = models.SqlViewsTrace(
                #     psk_uid=request.psk_uid,
                #     table_id=obj.id,
                #     api_request_payload=json.dumps(request_payload, indent=4),
                #     api_response_payload=json.dumps(response_data, indent=4)
                # )
                # db.add(trace_tbl)
                # db.commit()
        ...
        #     table_id=obj.id,
        #     api_request_payload=json.dumps(request_payload, indent=4),
        #     api_response_payload=json.dumps(response_list, indent=4)
        # )
        # db.add(trace_tbl)
        # db.commit()
...
@app.get('/sqlviews/api/v1/clone/{id}')
def clone_table_data(id: int, db: Session = Depends(get_db)):
    obj = db.query(models.SQLViewsMigrations).filter(models.SQLViews.id == id).first()
    return obj
