...
def custom_api(db, data):
    ...
    class ErpItemTransactionBcr(Base):
        __tablename__ = 'b2e_tbl_erp_itemtransactiondbcr'

        ITR_ID = Column(Integer, primary_key=True)
        ITR_DocID = Column(String)
        irn = Column(String)
        irnstatus = Column(String)

    with Session(mysql_engine) as mysql_db:
        obj = mysql_db.query(ErpItemTransactionBcr).filter(ErpItemTransactionBcr.ITR_DocID == credit_note_no).first()
        obj.irn = irn_value
        obj.irnstatus = "IRN_CANCELLED"

        mysql_db.add(obj)
        mysql_db.commit()
        mysql_db.refresh(obj)

        return {"message": "IRN Canceled Successfully"}
